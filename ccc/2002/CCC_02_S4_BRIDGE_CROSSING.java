	...
	public static void main (String[] t) throws IOException {
		...
		while (!q.isEmpty ()) {
			...
			if (curr.q_pos == Q - 1) {
				...
				if (dp[curr.q_pos + 1][1] == null || dp[curr.q_pos + 1][1].time + dp[curr.q_pos + 1][1].group_time >
						curr.time + curr.group_time + time[curr.q_pos + 1]) {
					dp[curr.q_pos + 1][1] = new State (curr.q_pos, curr.members, curr.group + 1, 1, curr.q_pos + 1,
					                                   curr.time + curr.group_time, time[curr.q_pos + 1]);
					q.offer (dp[curr.q_pos + 1][1]);
				}

				if (curr.members + 1 <= M) {
					if (dp[curr.q_pos + 1][curr.members + 1] == null || dp[curr.q_pos + 1][curr.members + 1].time +
							dp[curr.q_pos + 1][curr.members + 1].group_time > curr.time + Math.max (curr.group_time,
					                                                                                time[curr.q_pos +
							                                                                                1])) {
						dp[curr.q_pos + 1][curr.members + 1] = new State (curr.q_pos, curr.members, curr.group, curr
								.members + 1, curr.q_pos + 1, curr.time, Math.max (curr.group_time, time[curr.q_pos +
								1]));
