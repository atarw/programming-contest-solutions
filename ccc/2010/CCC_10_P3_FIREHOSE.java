...
public class CCC_10_P3_FIREHOSE {
  ...
  public static int hydrants (int K) {
    ...
    //(as long as house [i] is at a lesser or equal position to the hose that will be placed at house [i] - K in the for loop)
    for (int i = 0; i < houses.length && houses [i] <= houses [0] + K * 2; ++i) {
      count = 1; end = houses [i];//set one hose at house [i] - K
