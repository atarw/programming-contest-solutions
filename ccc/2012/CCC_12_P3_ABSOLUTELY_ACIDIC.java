...
public class CCC_12_P3_ABSOLUTELY_ACIDIC {
  
  public static void main (String [] args) throws IOException {
    BufferedReader in = new BufferedReader (new InputStreamReader (System.in));
    int N = Integer.parseInt (in.readLine ());
    NavigableMap <Integer, List <Integer>> map = new TreeMap <Integer, List <Integer>> ();//stores frequency to readings
    ...
    if (map.lastEntry ().getValue ().size () == 1) {//only one highest reading
      if (map.lowerEntry (map.lastKey ()).getValue ().size () == 1) {//only one second best
        System.out.println (Math.abs (map.lastEntry ().getValue ().get (0) - map.lowerEntry (map.lastKey ()).getValue ().get (0)));
      }
      else {
        List <Integer> list = map.lowerEntry (map.lastKey ()).getValue ();
        Collections.sort (list);
        int best = Math.max (Math.abs (map.lastEntry ().getValue ().get (0) - list.get (0)), Math.abs (map.lastEntry ().getValue ().get (0) - list.get (list.size () - 1)));
