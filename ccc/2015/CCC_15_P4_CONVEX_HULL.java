...
public class CCC_15_P4_CONVEX_HULL {
  ...
  public static void main (String [] tokens) throws IOException {
    ...
    for (int m = 0; m < M; m++) {
      tokens = in.readLine ().split (" ");
      a = Integer.parseInt (tokens [0]) - 1; b = Integer.parseInt (tokens [1]) - 1; t = Integer.parseInt (tokens [2]); h = Integer.parseInt (tokens [3]);
      ...
      list [a].add (new Edge (a, b, t, h)); list [b].add (new Edge (b, a, t, h));
    }
    ...
    while (!queue.isEmpty ()) {
      ...
      if (curr.C == B) {
        ...
        for (int e = 0; e < list [curr.C].size (); e++) {
          if (curr.H > list [curr.C].get (e).H && cache [list [curr.C].get (e).B][curr.H - list [curr.C].get (e).H] > cache [curr.C][curr.H] + list [curr.C].get (e).T) {
            cache [list [curr.C].get (e).B][curr.H - list [curr.C].get (e).H] = cache [curr.C][curr.H] + list [curr.C].get (e).T;
