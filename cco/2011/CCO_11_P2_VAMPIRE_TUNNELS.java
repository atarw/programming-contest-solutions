...
public class CCO_11_P2_VAMPIRE_TUNNELS {
	public static void main (String [] t) throws IOException {
		...
		for (int e = 0, start, end, dist, under; e < E; ++e) {
			t = in.readLine ().split (" ");
			start = Integer.parseInt (t [0]); end = Integer.parseInt (t [1]); dist = Integer.parseInt (t [2]); under = Integer.parseInt (t [3]);
		...
		while (!q.isEmpty ()) {
			...
			if (curr.curr == N - 1)
				...
				for (Edge e : list [curr.curr]) {
					if (curr.sun + e.D * e.U <= S && cache [curr.curr][curr.sun] + e.D < cache [e.E][curr.sun + e.D * e.U]) {
