...
public class CODEFORCES_275_D {

	static List<Integer>[] list;
	static long[] arr, plus, minus;
	...
	public static void dfs (int u, int last) {
		for (int v : list[u]) {
			if (v != last) {
				...
				minus[u] = Math.max (minus[u], minus[v]);
			}
		}

		long newv = arr[u] + plus[u] - minus[u];

		// need to set it back to 0
		if (newv > 0)
			minus[u] += newv;
		else
			plus[u] += Math.abs (newv);
	}
