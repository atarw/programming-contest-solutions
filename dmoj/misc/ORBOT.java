...
public class ORBOT {
	...
	public static Node merge (Node l, Node r) {
		...
		int pref = Math.max (l.pref, l.total | r.pref);
		int suf = Math.max (r.suf, r.total | l.suf);
		int best = Math.max (Math.max (l.best, r.best), l.suf | r.pref);
		return new Node (total, best, pref, suf);
	}
	...
	public static void update (int i, int l, int r, int arri, int v) {
		if (l == r) {
			arr[arri] = v;
	...
	public static void main (String[] t) throws IOException {
		...
		for (int q = 0; q < Q; ++q) {
			update (0, 0, N - 1, in.iscan () - 1, in.iscan ());
			out.println (query (0, 0, N - 1, 0, N - 1).best);
		}
	...
	private static class Node {
		...
		int total, best, pref, suf;

		public Node (int total, int best, int pref, int suf) {
			this.total = total;
			this.best = best;
			this.pref = pref;
			this.suf = suf;
