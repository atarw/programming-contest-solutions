...
public class TREE_CUTTING {
	...
	public static void main (String[] t) throws IOException {
		...
		for (int r = 0; r < R; ++r) {
			for (int c = 0; c < C; ++c) {
				if (max_height < maze[r][c] || max_height == maze[r][c] && Math.pow (sx - r, 2) + Math.pow (sy - c, 2)
						< Math.pow (sx - ex, 2) + Math.pow (sy - ey, 2)) {
					max_height = maze[r][c];
					ex = r;
					ey = c;
				}
		...
		while (!q.isEmpty ()) {
			...
			if (curr.x != ex || curr.y != ey) {
				...
				for (State s : next) {
					if (cache[s.x][s.y].h > cache[curr.x][curr.y].h + maze[s.x][s.y] || cache[s.x][s.y].h ==
							cache[curr.x][curr.y].h + maze[s.x][s.y] && cache[s.x][s.y].c > cache[curr.x][curr.y].c +
							(maze[s.x][s.y] == 0 ? 0 : 1)) {
						cache[s.x][s.y] = new State2 (cache[curr.x][curr.y].h + maze[s.x][s.y], cache[curr.x][curr.y]
								.c + (maze[s.x][s.y] == 0 ? 0 : 1));
						q.offer (s);
					}
				}
			}
