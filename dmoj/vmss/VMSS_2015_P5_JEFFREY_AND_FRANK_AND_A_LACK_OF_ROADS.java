...
public class VMSS_2015_P5_JEFFREY_AND_FRANK_AND_A_LACK_OF_ROADS {
  ...
  public static int solve (int R, int S, int curr) {
    if (cache [R][S][curr] == 0 && !(curr == 0 || R < 0 || S < 0)) {
      if (A [curr - 1].B > S || A [curr - 1].A > R) {
        ...
        cache [R][S][curr] = Math.max (A [curr - 1].V + solve (R - A [curr - 1].A, S - A [curr - 1].B, curr), solve (R, S, curr - 1));
      }
    }
  ...
  public static void main (String [] args) throws IOException {
    ...
    for (; n > 0;) {
      if (r - A [n - 1].A < 0 || s - A [n - 1].B < 0 || cache [r][s][n] - cache [r - A [n - 1].A][s - A [n - 1].B][n] != A [n - 1].V) {
        n--;
      }
      else {
        bought [n - 1]++;
