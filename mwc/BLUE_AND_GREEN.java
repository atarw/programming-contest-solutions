...
public class BLUE_AND_GREEN {
  ...
  public static void ways (int marbles, int M) {
    if (cache [marbles] < M) {//new config OR old config but new options
      ...
      if (M >= R) {
        ways (((marbles >>> 1) | ((marbles & (MASK >> N_MINUS_ONE)) << N_MINUS_ONE)) & MASK, M - R);//rotate right
      }
  ...
  public static void main (String [] t) throws IOException {
    ...
    N = Integer.parseInt (t [0]); int M = Integer.parseInt (t [1]);
    MASK = Integer.MAX_VALUE >> (31 - N);
    N_MINUS_ONE = N - 1;
    cache = new int [1 << N];
    
    t = in.readLine ().split (" ");
    S = Integer.parseInt (t [0]); L = Integer.parseInt (t [1]); R = Integer.parseInt (t [2]); I = Integer.parseInt (t [3]);
