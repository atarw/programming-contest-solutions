    ...
    while (!queue.isEmpty ()) {
      ...
      if (map.containsKey (curr)) {
        for (Integer i : map.get (curr)) {
          if (cache [curr - 1][i - 1] == 0 || cache [S - 1][i - 1] == 0 || cache [S - 1][i - 1] >= cache [S - 1][curr - 1] + PURSUIT_OF_KNOWLEDGE.T) {
            cache [S - 1][i - 1] = cache [S - 1][curr - 1] + PURSUIT_OF_KNOWLEDGE.T;
            queue.offer (i);
          }
          cache [curr - 1][i - 1] = PURSUIT_OF_KNOWLEDGE.T;
        }
      }
    }
  }
