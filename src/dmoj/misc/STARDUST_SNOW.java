...
public class STARDUST_SNOW {
	...
	public static int solve (int time, int pos, int temp, int cap) {
		...
		if (flakes [pos][time][1] != 0 && temp + flakes [pos][time][0] < B && cap + 1 <= K) {
			for (int move = M; move >= 0; --move) {
				if (pos - move >= 0) {
					cache [time][pos][temp][cap] = Math.max (cache [time][pos][temp][cap], flakes [pos][time][1] + solve (time + 1, pos - move, temp + flakes [pos][time][0], cap + 1));
				}
		...
		for (int move = M; move >= 0; --move) {
			if (pos - move >= 0) {
				cache [time][pos][temp][cap] = Math.max (cache [time][pos][temp][cap], solve (time + 1, pos - move, temp, cap));
			}
	...
	public static void main (String [] t) throws IOException {
		...
		R = in.iscan (); C = in.iscan (); S = in.iscan (); B = in.iscan (); K = in.iscan (); M = in.iscan ();
		
		int T_i, V_i, C_i, R_i;
		
		for (int s = 0; s < S; ++s) {
			T_i = in.iscan (); V_i = in.iscan (); C_i = in.iscan (); R_i = in.iscan ();
