...
public class MACKENZIE_NEW_YEARS_CHALLENGE_P5_JELLY {
  ...
  public static int solve () {
    ...
    while (!queue.isEmpty ()) {
      ...
      if (curr.X == 0 || curr.Y == 0 || curr.Z == 0 || curr.X == maze [0][0].length - 1 || curr.Y == maze [0].length - 1 || curr.Z == maze.length - 1) {
      ...
      for (P3D u : n) {
        if (steps [u.Z][u.Y][u.X] > steps [curr.Z][curr.Y][curr.X] + maze [u.Z][u.Y][u.X] || !u.equals (j) && steps [u.Z][u.Y][u.X] == 0) {
...
class P3D implements Comparable <P3D> {
  ...
  public String toString () {
    return "(" + X + ", " + Y + ", " + Z + ") " + MACKENZIE_NEW_YEARS_CHALLENGE_P5_JELLY.steps [Z][Y][X];
  }
  
  public int compareTo (P3D p) {
    return MACKENZIE_NEW_YEARS_CHALLENGE_P5_JELLY.steps [Z][Y][X] - MACKENZIE_NEW_YEARS_CHALLENGE_P5_JELLY.steps [p.Z][p.Y][p.X];
  }
