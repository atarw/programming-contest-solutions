  ...
  public static void main (String [] t) throws IOException {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      out: for (int n1 : spin) {
        for (int n2 : spin) {
          for (int f = 0; f < 5; f++) {
            if (targets [f] != -1 && spin.contains (targets [f] - n1 - n2) || spin.contains (targets [f] - n1 * n2) || targets [f] % (n1 + n2) == 0 && spin.contains (targets [f] / (n1 + n2)) || targets [f] % (n1 * n2) == 0 && spin.contains (targets [f] / (n1 * n2))) {
              targets [f] = -1;
              found++;
