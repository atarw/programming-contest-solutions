...
public class checklist {
	...
	public static int solve (int x, int y, int b, int h, int g) {
		...
		return dp [h][g][b] = Math.min (h_dist + solve (hx [h + 1], hy [h + 1], 1, h + 1, g), g_dist + solve (gx [g + 1], gy [g + 1], 0, h, g + 1));
	}
	...
	public static void main (String [] t) throws IOException {
		...
		PrintWriter out = new PrintWriter ("checklist.out");
	 	...
	 	H = in.iscan (); G = in.iscan ();
		hx = new int [H + 1]; hy = new int [H + 1];
		gx = new int [G + 1]; gy = new int [G + 1];
		
		for (int h = 0; h < H; ++h) {
			hx [h + 1] = in.iscan (); hy [h + 1] = in.iscan ();
		}
		
		for (int g = 0; g < G; ++g) {
			gx [g + 1] = in.iscan (); gy [g + 1] = in.iscan ();
		}
