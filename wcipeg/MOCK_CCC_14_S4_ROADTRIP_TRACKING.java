...
public class MOCK_CCC_14_S4_ROADTRIP_TRACKING {
	...
	public static int djikstra (boolean first) {
		...
		while (dp [u].prev != -1) {
			edges [dp [u].prev].w = edges [dp [u].prev < edges.length / 2 ? dp [u].prev + edges.length / 2 : dp [u].prev].w = 1;
			u = edges [dp [u].prev].u;
			++length;
		}
	...
	public static void main (String [] t) throws IOException {
		...
		for (int m = 0, a, b; m < M; ++m) {
			...
			edges [m] = new Edge (a, b, 0); edges [m + M] = new Edge (b, a, 0);
			list [a].add (m); list [b].add (m + M);
		}
	...
	private static class State {
		...
		public State (int u, int dist, int prev) {
			this.u = u; this.dist = dist; this.prev = prev;
		}
	}
